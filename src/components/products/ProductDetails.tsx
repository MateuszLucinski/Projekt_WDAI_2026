...
function ProductDetails() {
  ...
  const handleAddToCart = () => {
    if (product) {
      addToCart(
        ...
        quantity,
      ); // Przekazujemy aktualny stan quantity
  ...
  useEffect(() => {
    fetch(`http://localhost:3002/api/items/${id}/reviews`)
      .then((res) => {
        if (!res.ok) {
          if (res.status === 404) {
            setReviews([]);
        ...
        <Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
            ...
            <Typography variant="body1" color="text.secondary">
              {averageRating.toFixed(1)} ({reviewsCount} opinii)
            </Typography>
          </Box>
